...
@lru_cache(maxsize=1)
def get_model_bundle() -> tuple[WhisperModel, dict[str, str]]:
    configure_runtime()
...
def get_model() -> WhisperModel:
    return get_model_bundle()[0]


def _transcribe_worker(audio_path: str, language: str | None, result_queue: mp.Queue) -> None:
    try:
        result_queue.put({"type": "status", "stage": "loading_model"})
        model, model_meta = get_model_bundle()
...
def transcribe_audio(
    ...
    process = context.Process(target=_transcribe_worker, args=(str(audio_path), language, result_queue))
    process.start()
